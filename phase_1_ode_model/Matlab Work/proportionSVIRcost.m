...
%against measles, using the SVIR model as a basis for the number of
...
r0 = 53000000;
u0 = [s0;v0;i0;r0];
U = solveSVIR (u0,massvaccprop, intromassvaccyear, (intromassvaccyear + massvaccperiod));
%We don't care about the cost we incur in the past so can ignore the past.
postmassvacc = U(:,end);
while (c<=l)
    p = prop(c);
    temp = solvenewinfectiousandvaccSVIR(postmassvacc,p,0,50);

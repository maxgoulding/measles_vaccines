%Solves the realistic BSVIR model using the fourth stage Runge-Kutta
%method. Storing the partially age stratified model as a matrix
%It then returns the matrix storing the population.
%n - the number of years to run the system
%vage - the age at which vaccination schedual is applied
...
%h - time step
%c - counter
%trackage - the age up to which age group knowledge is stored. In this case
%it is 4. This must be greater than vage.
%vagegroup - the age group up to which group knowledge is stored. 
%introvaccstep - the step at which vaccine is introduced.
%vagegroup - the age group at which vaccination is applied. 
%l - length of t
...
%mothers that lose immunity between time steps.
function y = solverealisticBSVIRpop(u0,p,vage,vyear,n)
c = 1;
h = 1/365;
t = [0:h:n];
l = length(t);
trackage = 4;
trackagegroup = trackage/h;
...
population = zeros(6,trackagegroup+1);
population(:,end) = u0;
lossofunvaccimmunityvalues = zeros (1,trackagegroup);
for i=1:length(lossofunvaccimmunityvalues)
   lossofunvaccimmunityvalues(i) =  lossofunvaccinatedimmunity(i*h,(i+1)*h);
end
lossofvaccimmunityvalues = zeros (1,trackagegroup);
...
while (c<=l)
    ...
    births = (h*(kbirth1+(2*kbirth2)+(2*kbirth3)+kbirth4)/6);
    %Apply Vaccination
    population(4,vagegroup) = p*population(3,vagegroup);
    population(3,vagegroup) = (1-p)*population(3,vagegroup);
    %Runge-Kutta Method for change in population dynamics
    k1 = matrixBSVIR(t(c),population);
    k2 = matrixBSVIR(t(c)+h/2,population+(h*k1/2));
    k3 = matrixBSVIR(t(c)+h/2,population+(h*k2/2));

%Determines the cost of double vaccinating varying proportions of the population
%against measles, using the realistic BSVIR model as a basis for the number of
%infections and vaccinations occured. The system is run for 10 years
%without vaccination, single vaccination is then applied at 85% for 20 years to
...
%Parameters
%vage - the age in which vaccination schedule is administered.
...
%massvaccage - the age at which the population is vaccinated during mass
%vaccination. Assumed to be 1 and with a single vaccination.
%the mass period.
...
function y = proportionrealisticdoubleBSVIRcost (lowvage,highvage)
...
%More Efficient as avoids recalculation 100 times.
postmassvaccpop = solverealisticBSVIRpop(u0,massvaccprop,massvaccage,intromassvaccyear, (intromassvaccyear + massvaccperiod));
while (c<=l)
    p = prop(c);
    temp = solvenewinfectiousandvaccrealisticdoubleBSVIR(postmassvaccpop,p,lowvage,highvage,0,50);

...
%br0 - intial number infants with maternal immunity recovered mother.
%bv0 - intial number infants with maternal immunity vaccinated mother.
...
%h - time step
%c - counter
%introvaccstep - the step at which vaccine is introduced and we switch from
%SIR dynamics to SVIR.
...
%mothers that lose immunity between time steps.
function y = solvematrixBSVIR(p,vage,vyear,n)
%Intial Values
Br0 = 0;
Bv0 = 0;
s0 = 6400000;
...
population = zeros(6,vagegroup+1);
population(:,end) = u0;
lossofunvaccimmunityvalues = zeros (1,vagegroup);
for i=1:length(lossofunvaccimmunityvalues)
   lossofunvaccimmunityvalues(i) =  lossofunvaccinatedimmunity(i*h,(i+1)*h);
end
lossofvaccimmunityvalues = zeros (1,vagegroup);
